...
def main():
    print("--- 🌟 ARGIA SOLAR MONITORING v4.9 (Integrated Logic) ---")
    service = get_service()
    ...
    for row in config_rows:
        if len(row) >= 10:
            ...
            s_id = str(row[9]).strip() # SiteID w kolumnie J
    ...
    # 4. Składanie danych do zapisu
    final_data = []
    for p_key, conf in plants_config.items():
        energy = all_prod.get(p_key, 0.0)
    ...
    if final_data:
        try:
            ...
            print(f"✅ Sync complete for {date_slash}. Data saved.")
        except Exception as e:
