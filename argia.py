...
# --- MODUŁ GROWATT (Maksymalnie uproszczony) ---
def fetch_growatt_data_v2(plant_id, date_str):
    ...
    # Zamiast w __init__, ustawiamy serwer bezpośrednio na obiekcie
    ...
    try:
        ...
        # Pobranie danych (timespan=3 to dane dzienne)
        data = api.plant_detail_info(plant_id, timespan=3, date=date_str)
        
        # Wyciąganie energii z różnych możliwych pól w JSON
        energy = data.get('daily_energy') or data.get('energy') or 0
        return float(energy)
    except Exception as e:
        # Jeśli błąd to 403, spróbujemy wypisać więcej info
...
def main():
    ...
    for p in plants:
        ...
        # Debugging: sprawdzamy co faktycznie jest w SiteID
