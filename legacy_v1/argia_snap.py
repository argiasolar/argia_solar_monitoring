...
Key change vs the broken unified version:
✅ Growatt is fetched EXACTLY like your working Growatt snapshot script:
   - warm context
   - load MAX + Inverter HTML
   - discover AJAX endpoints
   - probe safe list endpoints
   - extract EToday from working keys

Notes:
- Cloud_Coverage is written as 0..1 fraction. If the sheet formats it as %, you will see 43.50% etc.
- UpdateTime is always Mexico City time string "YYYY-MM-DD HH:MM:SS".
"""
...
def parse_update_time_to_mx(val: Any) -> str:
    """
    Growatt sometimes returns:
      - "2026-02-07 10:24:17"
      - epoch seconds
      - epoch ms
    We normalize to MX time string.
    """
    ...
    # already formatted: assume MX local
    return s
...
# Meteo cache
# ----------------------------
def looks_like_huawei_station_or_growatt_siteid(siteid: str) -> bool:
    return looks_like_huawei_station(siteid) or looks_like_growatt_siteid(siteid)


# key=(weatherPlantId, weather_sn, addr)
...
# ----------------------------
# Growatt client (COPIED approach from your working file)
# ----------------------------
@dataclass
class GrowattAuth:
    user: str
...
class GrowattMonitoringClient:
    ...
    def fetch_devices_for_plant(self, plant_id: str, page_size: int, max_pages: int) -> Tuple[List[Dict[str, Any]], str]:
        ...
        for ep in safe_candidates:
            ...
            if any(pick(it, ["sn", "deviceSn", "invSn", "serialNum", "serialNo"]) for it in all_items):
                GLOG.info("✅ Using endpoint %s (items=%d) for plant %s", ep, len(all_items), plant_id)
                return all_items, ep

        GLOG.warning("❌ No device list endpoint produced SNs for plant %s", plant_id)
        return [], ""
...
def growatt_extract_etoday(it: Dict[str, Any]) -> Optional[float]:
    # EXACT key set from your working script (plus a couple of common variants)
...
# ----------------------------
# Huawei (your stable previous version)
# ----------------------------
...
def main() -> None:
    ...
    if g_user and g_pass:
        ...
        for srow in snap_rows:
            ...
            items, endpoint = gcli.fetch_devices_for_plant(siteid, page_size=page_size, max_pages=max_pages)
            ...
            if items and endpoint:
                GLOG.info("Plant %s using %s -> items=%d", siteid, endpoint, len(items))

            # Debug mismatch: show a few returned SNs
            if items and wanted_sns and not any(sn in fetched for sn in wanted_sns):
                sample = [normalize_sn(pick(it, ["sn", "deviceSn", "invSn", "serialNum"]) or "") for it in items[:8]]
                GLOG.warning("Plant %s: SNAP SNs did not match returned list. Sample returned SNs=%s", siteid, sample)
            ...
            for sn in wanted_sns:
                ...
                if it:
                    ...
                    # still write the row, but clearly missing data
                    rows_out.append([
                        extracted_at,
                        now_mx_str(),
                        siteid,
