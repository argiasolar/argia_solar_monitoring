...
| A number looks wrong and you want the document behind it (v250) | open the page, click the ⎘ / ↗ in the row's **Source** column, or read the **Where to change this** box on a project or cost-centre page | every row stores the file, sheet and row it was read from (`src_sheet`, `src_row`, `src_gid` + `fin_source_file.mime`). A Google Sheet (the ARGIA PROJECT workbooks) links to the cell; an .xlsx in Drive (tracker, overview, the accountants' prints) opens the file with the sheet and row in the tooltip; with no Drive id yet the link is a Drive search. The books are marked **read-only** — a wrong booked figure is corrected in CONTPAQi and arrives with the next close. New columns reach an existing database through `scripts/fin_schema.py --apply` (`ADD COLUMN IF NOT EXISTS`, idempotent; run it before the ingest after any upgrade). Pointers fill in on the next `fin_drive_ingest.py --apply`; a file unchanged by hash still gets its Drive id refreshed, so the links go live the moment the folders are shared |
