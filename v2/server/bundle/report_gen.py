...
try:
    for r in q("SELECT plant_key, inverter_sn, sum(minutes_over_65), sum(events), max(peak_c),"
               ...
               " count(*) FILTER (WHERE cooling_health = 'POOR'), sum(coalesce(vendor_derating_minutes, 0))"
        ...
        if len(r) >= 11:
            thermal30.setdefault(r[0], {})[r[1]] = {
                ...
                'days': int(f(r[8]) or 0), 'poor_days': int(f(r[9]) or 0), 'vendor_min': int(f(r[10]) or 0)}
except RuntimeError:
    thermal30 = {}
...
def thermal_card(k):
    ...
    rows, t_lost, t_min65, t_der, t_vendor = [], 0.0, 0, 0, 0
    for sn in sorted(stats, key=lambda s: -(stats[s]['lost'] or 0)):
        a = stats[sn]
        label, _rated = inv_meta.get((k, sn)) or (sn, 0)
        vendor_min = a.get('vendor_min', 0)
        cls = ('bad' if (a['poor_days'] >= 3 or a['lost'] >= 50 or vendor_min >= 60)
               else 'warn' if (a['poor_days'] or a['min65'] >= 300 or vendor_min) else '')
        ...
        rows.append(f'<tr><td>{_naming.inverter_html(label, sn)}</td>'
                    ...
                    f'<td class="num">{("%.1f" % (vendor_min / 60)) if vendor_min else "—"}</td>'
    ...
    return (f'<div class="card"><h2 style="display:flex;align-items:center">{t("Thermal health — last 30 days","Salud térmica — últimos 30 días")}{tip}</h2>'
            ...
            f'<th class="num">{t("Vendor h","Fabricante h")}</th>'
...
<div class="tile"><div class="tlabel">{t("This month","Este mes")}</div>
 ...
 <div class="tsub">{this_m} → {asof[8:]}</div></div>
    ...
    ic_map = ('<svg viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">'
...
<a href="portfolio/">{ic_map}{t("Portfolio map","Mapa del portafolio")}</a>
