...
v263: moved to tests/portal and onto the private cluster from conftest.py
(its own database, argia_fin_test). Until v263 it needed a hand-made
database named in ARGIA_FIN_TEST_DB and therefore never ran on CI or on
the laptop; now it runs wherever the portal tests run, and CI requires it.
"""
...
DB = "argia_fin_test"
ENV: dict = {}                    # the private cluster's env, set by the pipeline fixture
...
@pytest.fixture
def in_db(pg_env, monkeypatch):
    """In-process code (the Flask app) talks to the private cluster too."""
    for k in ("PGHOST", "PGPORT", "PGUSER", "PGTZ", "PATH"):
        monkeypatch.setenv(k, pg_env[k])
    monkeypatch.setenv("ARGIA_PG_DB", DB)


def run(script: str, *args: str, env=None, ok=(0,)) -> str:
    e = dict(ENV, ARGIA_PG_DB=DB, PYTHONPATH=str(V2))
    e.update(env or {})
...
def pipeline(pg_env):
    ENV.clear()
    ENV.update(pg_env)
    subprocess.run(["dropdb", "--if-exists", "--force", DB], env=ENV, check=True, capture_output=True)
    subprocess.run(["createdb", DB], env=ENV, check=True, capture_output=True)
    subprocess.run(["runuser", "-u", "postgres", "--", "psql", "-d", DB, "-q", "-v", "ON_ERROR_STOP=1", "-f", str(V2 / "server/bundle/schema.sql")], check=True, capture_output=True, env=ENV)
    assert "44 missing" in run("fin_schema.py", ok=(2,))          # exit 2 = missing tables, by contract (28 v243 + 14 v245 + 1 v247)
...
class TestPipeline:
    ...
    def test_the_app_queries_run(self, pipeline, in_db):
...
@pytest.fixture(scope="module")
def books(pipeline, tmp_path_factory):
...
class TestBooksPipeline:
    ...
    def test_the_books_pages_run_on_postgres(self, books, in_db):
        sys.path.insert(0, str(V2 / "server" / "bundle"))
        import importlib
        F = importlib.import_module("fin_app")
        F.app.config.pop("ROWS", None)
